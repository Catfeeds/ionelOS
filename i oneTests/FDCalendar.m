...
- (UIDatePicker *)datePicker {
    if (!_datePicker) {
        ...
        _datePicker.center=CGPointMake(self.bounds.size.width/2, frame.size.height/2);
    }
...
- (void)setupTitleBar {
    ...
    [leftButton setImage:[UIImage imageNamed:@"箭头"] forState:UIControlStateNormal];
...
- (void)setupScrollView {
    ...
    [self.scrollView setFrame:CGRectMake(0, 75
...
- (void)showDatePickerView {
    ...
    CGFloat m=   CGRectGetMaxY(self.bounds);
    
    _datePickerView = [[UIView alloc] initWithFrame:CGRectMake(0,-m, self.frame.size.width, m)];
    _datePickerView.backgroundColor = [UIColor whiteColor];
    _datePickerView.clipsToBounds = YES;
    
     [self addSubview:_datePickerView];
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(0, _datePickerView.bounds.size.height-30, 30, 30)];
    ...
    UIButton *okButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 30, _datePickerView.bounds.size.height-30, 30, 30)];
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.datePickerView.transform=CGAffineTransformMakeTranslation(0, m);
    }];
}
...
- (void)selectCurrentDate {
    ...
    if ([self.mydelegete respondsToSelector:@selector(passDate:)]) {
        [self.mydelegete passDate:self.datePicker.date];
    }
    
    if ([self.mydelegete respondsToSelector:@selector(requestLineData)]) {
        [self .mydelegete requestLineData];
    }
